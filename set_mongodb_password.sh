...
mongosh admin --eval "db.createUser({user: '$USER', pwd: '$PASS', roles:[  {role: 'root', db: 'admin'}  ]});"

if [ "$DATABASE" != "admin" ]; then
    echo "=> Creating an ${USER} user with a ${_word} password in MongoDB"
    mongosh admin -u $USER -p $PASS << EOF
...
echo ""
echo "    mongosh $DATABASE -u $USER -p $PASS --host <host> --port <port>"
echo ""
